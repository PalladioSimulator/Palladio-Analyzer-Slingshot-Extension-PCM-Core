 ...
 * 
 * @author Julijan Katic, Steffen Becker, Sebastian Lehrig, Matthias Becker
 */
public final class MonitorRepositoryUtil {

	/**
	 * Method checks if given element should be monitored with given performance
	 * metric. If yes, it will return the corresponding MeasurementSpecification,
	 * otherwise null.
	 * 
	 * @param monitorRepositoryModel the monitoring model.
	...
	public static String getMeasurementIdentifier(final MeasuringPoint measuringPoint) {
		return new PcmmeasuringpointSwitch<String>() {
	...
	public static EObject getMonitoredElement(final MeasuringPoint mp) {
		...
		if (eObject == null) {
			...
			if (eObject == null) {
				throw new IllegalArgumentException("Could not find EObject for Measuring Point \""
						...
						+ "or getEObjectFromGeneralMeasuringPoint method.");
			}
		}
