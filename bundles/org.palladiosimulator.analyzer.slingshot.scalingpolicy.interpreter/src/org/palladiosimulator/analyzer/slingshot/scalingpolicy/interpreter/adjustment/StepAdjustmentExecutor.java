...
/**
 * Copies exactly the number of times specified in the {@link StepAdjustment#getStepValue()}
 * model element.
 *  
...
public final class StepAdjustmentExecutor extends AbstractAdjustmentExecutor<StepAdjustment> {
	...
	public StepAdjustmentExecutor(final StepAdjustment adjustment, final SimulationInformation simulationInformation) {
	...
	public AdjustmentResult onTrigger(final TriggerContext triggerContext) {
		...
		final List<ResourceContainer> newResourceContainers = new ArrayList<>(
				environment.getResourceContainer_ResourceEnvironment().size() * this.getAdjustmentType().getStepValue());
