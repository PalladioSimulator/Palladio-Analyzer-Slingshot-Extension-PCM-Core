...
public class ScalingPolicyDefinitionInterpreter extends SpdSwitch<List<TriggerContext>> {
	...
	public List<TriggerContext> caseScalingPolicy(final ScalingPolicy scalingPolicy) {
		...
		final TriggerContext.Builder contextBuilder = triggerContextBuilder.withAdjustmentExecutor(adjustmentExecutor)
				...
				.withTargetGroup(scalingPolicy.getTargetGroup());

		final ScalingTriggerInterpreter scalingTriggerInterpreter = new ScalingTriggerInterpreter(this.engine,
				contextBuilder);
		final ScalingTriggerPredicate scalingTriggerPredicate = scalingTriggerInterpreter
				.doSwitch(scalingPolicy.getScalingTrigger());
		contextBuilder.withScalingTriggerPredicate(scalingTriggerPredicate)
				.withScalingTrigger(scalingPolicy.getScalingTrigger());

		final ConstraintInterpreter policyConstraintInterpreter = new ConstraintInterpreter();
