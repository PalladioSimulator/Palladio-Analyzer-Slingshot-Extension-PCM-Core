...
public class EMFResourceSetInitializerHelper {
	...
	public static void initEMF() {
		...
		registry.getExtensionToFactoryMap().put(ModelTypeConstants.REPOSITORY_EXTENSION,
				new RepositoryResourceFactoryImpl());
		registry.getExtensionToFactoryMap().put(ModelTypeConstants.RESOURCE_ENVIRONMENT_EXTENSION,
		...
		registry.getExtensionToFactoryMap().put(ModelTypeConstants.ALLOCATION_EXTENSION,
				new AllocationResourceFactoryImpl());
		registry.getExtensionToFactoryMap().put(ModelTypeConstants.RESOURCETYPE_EXTENSION,
				new ResourcetypeResourceFactoryImpl());
		registry.getExtensionToFactoryMap().put(ModelTypeConstants.USAGEMODEL_EXTENSION,
...
//        , SeffReliabilityPackage.eINSTANCE
		...
		/**
		 * A remapping of the URI pathmap://... must be done in the plugin.xml of this
		 * plugin public static final URI PCM_PALLADIO_RESOURCE_TYPE_URI =
		 * URI.createURI("pathmap://PCM_MODELS/Palladio.resourcetype"); public static
		 * final URI PCM_PALLADIO_PRIMITIVE_TYPE_REPOSITORY_URI =
		 * URI.createURI("pathmap://PCM_MODELS/PrimitiveTypes.repository");
		 * 
		 * otherwise the resource loading in PreparePCMBlackboardPartitionJob l. 45/46
		 * is not working
		 * 
		 * Configured mapping: (ref.
		 * https://www-01.ibm.com/support/docview.wss?uid=swg21281749) - register
		 * extension ' org.eclipse.emf.ecore.uri_mapping' - add remapping:
		 * pathmap://PCM_MODELS/PrimitiveTypes.repository to
		 * platform:/plugin/org.palladiosimulator.pcm.resources/defaultModels/PrimitiveTypes.repository
		 * - add remapping: pathmap://PCM_MODELS/Palladio.resourcetype to
		...
		uriMap.put(URI.createURI(ModelTypeConstants.PALLADIO_RESOURCETYPES_PATHMAP),
				URI.createURI(ModelTypeConstants.PALLADIO_RESOURCETYPES_PATHMAP_TARGET));
		uriMap.put(URI.createURI(ModelTypeConstants.PRIMITIVE_TYPES_REPO_PATHMAP),
				URI.createURI(ModelTypeConstants.PRIMITIVE_TYPES_REPO_PATHMAP_TARGET));
