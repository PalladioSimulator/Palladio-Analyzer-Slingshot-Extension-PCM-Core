 ...
 */
/**
 * This class evaluates all the extensions that are defined through the behaviour extension point.
 * 
 * @author Julijan Katic
 */
public class BehaviourExtensionsHandler {
	
	private static final String BEHAVIOUR_EXTENSION_POINT_ID = "org.palladiosimulator.analyzer.slingshot.extensionpoint.behaviour";
	
	private static final Logger LOGGER = Logger.getLogger(BehaviourExtensionsHandler.class);
	
	/**
	 * This method will return all providers of every extension that is compatible to the extension point.
	 * @return the list of all providers. If there are no valid extensions or a exception occurs, an empty list
	 * 			will be returned.
	 */
	public static List<DecoratedSimulationBehaviorProvider> getAllProviders() {
		...
		try {
			final IConfigurationElement[] config = Platform.getExtensionRegistry().getConfigurationElementsFor(BEHAVIOUR_EXTENSION_POINT_ID);
