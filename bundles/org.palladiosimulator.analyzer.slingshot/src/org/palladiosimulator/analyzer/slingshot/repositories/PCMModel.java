...
/**
 * This class encapsulates a PCM model and provides a convenient loading
 * mechanism. A PCM model comprises several partial models:
 * <ul>
...
public class PCMModel {
	...
	private static enum PCMPartialModel {
		USAGE(UsagemodelPackage.eINSTANCE.getUsageModel()), ALLOCATION(AllocationPackage.eINSTANCE.getAllocation()),
	...
	/**
	 * Default constructor. This constructor is not intended to be used from outside
	 * this class. Instead, the {@code loadFromBundle()} method is supposed to be
	 * used.
	 */
	...
	/**
	 * Loads a PCM model that is contained in the specified bundle. The loading
	 * procedure requires the location of two model files: the usage model and the
	 * allocation model.
	 * 
	 * @param bundle                  the bundle containing the model files that are
	 *                                to be loaded
	 * @param usageModelLocation      the location of the usage model file
	 * @param allocationModelLocation the location of the allocation model file
	 * @return
	 */
	 ...
	 * allocation model.
	 * 
	 * @param usageModelUri      the location of the usage model file in form of a
	 *                           URI
	 * @param allocationModelUri the location of the allocation model file in form
	 *                           of a URI
	 * @return
	 */
	public static PCMModel loadFromUri(final URI usageUri, final URI allocationUri) {
	...
	/**
	 * Creates an absolute bundle {@link URI} from the specified relative
	 * {@code path}.
	 * 
	 * @param bundle the bundle that contains the file specified by {@code path}
	 * @param path   the relative path
	 * @return the absolute URI of {@code path} pointing to a file in the bundle or
	 *         null, if there is no such file in the bundle
	 */
	...
	public void saveToFolder(final URI folder) throws IOException {
		// check argument
		if (!folder.isFile()) {
		...
		final ResourceSet resourceSet = allocationModel.eResource().getResourceSet();
		for (final Resource resource : resourceSet.getResources()) {
			saveResourceToFolder(resource, folder);
		}
	}

	private void saveResourceToFolder(final Resource resource, final URI folder) throws IOException {
		// build path
		final String fileName = resource.getURI().lastSegment();
		final URI pathURI = folder.appendSegment(fileName);

		// write model to file
		final OutputStream fos = new FileOutputStream(pathURI.toFileString());
