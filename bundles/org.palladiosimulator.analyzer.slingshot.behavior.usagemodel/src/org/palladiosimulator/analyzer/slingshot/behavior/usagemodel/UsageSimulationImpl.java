...
public class UsageSimulationImpl implements SimulationBehaviourExtension {
	...
	public ResultEvent<DESEvent> onSimulationStart(final SimulationStarted evt) {
		...
		if (usageInterpretationContext.isClosedWorkload()) {
			...
			for (int i = 0; i < workloadSpec.getPopulation(); i++) {
				final UsageScenarioInterpreter<Object> interpreter = new UsageScenarioInterpreter<>(new User(), new UserInterpretationContext(usageScenario, firstAction));
	...
	public ResultEvent<DESEvent> onWakeUpUserEvent(final UserWokeUp evt) {
		final UsageScenarioInterpreter<Object> interpreter = new UsageScenarioInterpreter<>(evt.getEntity(), evt.getUserInterpretationContext());
		interpreter.continueInterpretation();
		final DESEventSet events = interpreter.getSideEffectEvents();
		return ResultEvent.ofAll(events);
	}
	
	@Subscribe
	public ResultEvent<DESEvent> onFinishUserRequest(final UserRequestFinished evt) {
		final UsageScenarioInterpreter<Object> interpreter = new UsageScenarioInterpreter<>(evt.getEntity().getUser(), evt.getUserContext());
		interpreter.continueInterpretation();
		final DESEventSet events = interpreter.getSideEffectEvents();
		return ResultEvent.ofAll(events);
	}
