...
@OnEvent(when = RequestInitiated.class, then = JobInitiated.class, cardinality = EventCardinality.SINGLE) //TODO: Implement resourcesimulation
...
//			userRequestInit.getUserContext() this needs to be returned upon all the request are finished. 
			...
			//final ResultEvent<DESEvent> result = ResultEvent.ofAll(Set.of(new JobInitiated(new Job(0, null, false, 10.0, request), 0)));
			final ResultEventBuilder<DESEvent> builder = ResultEvent.createResult();
			// builder.addEvent(new JobInitiated(new Job(0, null, false, 10.0, request), 0));
